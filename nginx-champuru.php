...
private function flush_this()
{
    ...
    $url = str_replace("@ktai", "", $url);
    $url = str_replace("@smartphone", "", $url);
    foreach([$url, $url.'@ktai', $url.'@smartphone'] as $uri) {
    	$key    = $this->get_cache_key($uri);
    	$caches = $this->get_cache($key, $uri);
...
private function flush_cache()
{
	...
	foreach([$url, $url.'@ktai', $url.'@smartphone'] as $uri) {
            if ($uri) {
                do_action('nginxchampuru_flush_cache', $uri);
            }
            $caches = $this->get_cache($key->cache_key, $uri);
            foreach ((array)$caches as $cache) {
                if (is_file($cache)) {
                    unlink($cache);
            	}
...
public function get_the_url()
{
    ...
    $url .= isset($_SERVER["HTTP_X_UA_DETECT"]) ? $_SERVER["HTTP_X_UA_DETECT"] : '';
